...
class Node:
    ...
    def __str__(self):
        return str([self._name, "Work Time: ", str(self.work_time), "Parents:", self.parents])
...
class Worker:
    ...
    def finish_part(self):
        if self.check_if_finished():
            finished_part = []
            finished_part.append(self.part)
            self._reset()
            return finished_part[0]
...
def piece_time(piece):
    ...
    return letters.index(piece) + time + 1
...
while len(done_pile) < len(sequence):
    parts_refresh = refresh_pile(pieces, done_pile)
    active_parts = parts_refresh[0]
    active_parts.sort(key=sort_name)
    incomplete_parts_list_copy = parts_refresh[1]
    for worker in workers_list:
        if len(active_parts) > 0 and not worker.has_part():
            worker.take_part(active_parts.pop(0))
        if worker.has_part():
            worker.do_work()
            if worker.check_if_finished():
                finished_part = worker.finish_part()
                done_pile += finished_part.get_name()
    total_time += 1
    pieces = incomplete_parts_list_copy

    # Visualization block
    if PRINT_TEST_VISUALIZATION:
        my_string = ''
...
print(total_time)
print(done_pile)
print("HPXYDLOTCGNQSEMIAZBKRUWVFJ is expected sequence") #obtained from another user using my input for debugging
